...
fn build_ascii_character_vector(special_characters: bool) -> Vec<u32> {
    ...
    let mut character_code: u32 = 33;

    while character_code < 127 {

        if character_code == 34 {
            character_code += 1;
            continue;
        }

        if special_characters {
            if char::from_u32(character_code).unwrap().is_ascii_graphic()
                && !char::from_u32(character_code).unwrap().is_ascii_alphanumeric() {
                vector.push(character_code);
            }
        }
        else {
            if char::from_u32(character_code).unwrap().is_ascii_alphanumeric() {
                vector.push(character_code);
            }
        }
...
fn add_special_characters(password_length: u8, password_string: String) -> String {
    let mut rng = rand::rng();
    let mut rand_index: u32;
    let mut password_vector: Vec<char> = password_string.chars().collect();
    ...
    let special_character_limit: u8 = rng.random_range(1..(password_length / 2));
    ...
    while special_character_count < special_character_limit {
        rand_index = rng.random_range(..password_vector.len()) as u32;
        character = password_vector[rand_index as usize];
        if character.is_ascii_graphic() && !character.is_ascii_alphanumeric() {
            continue;
        }
        else {
            let graphic_index = rng.random_range(..character_vector.len()) as u32;
            password_vector[rand_index as usize] = char::from_u32(character_vector[graphic_index as usize]).unwrap();
            special_character_count += 1;
        }
    }

    return password_vector.into_iter().collect();
}
